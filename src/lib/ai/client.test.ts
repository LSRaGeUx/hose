...
const env = vi.hoisted(() => ({
  HOSE_ANTHROPIC_API_KEY: undefined as string | undefined,
}))
vi.mock('#/env', () => ({ env }))
...
beforeEach(() => {
  vi.resetModules()
  env.HOSE_ANTHROPIC_API_KEY = undefined
})
...
describe('getAnthropic', () => {
  ...
  it('names Netlify explicitly when handed a gateway JWT', () => {
    ...
    env.HOSE_ANTHROPIC_API_KEY = 'eyJhbGciOi.eyJpc3MiOiJuZXRsaWZ5LWFwaSI.c2ln'
  ...
  it('rejects anything else that is not an Anthropic key', () => {
    env.HOSE_ANTHROPIC_API_KEY = 'not-a-key'
    expect(() => getAnthropic()).toThrow(/sk-ant-/)
  })

  it('accepts a direct key', () => {
    env.HOSE_ANTHROPIC_API_KEY = 'sk-ant-test-key'
